...
import { updateAccount } from "@/features/accounts/services/accountService"
...
function getAccountTypeValue(type: string) {
...
function getAccountColorValue(account: Account) {
    const color = accountColors.find((item) =>
        account.color.includes(item.value),
    )

    return color?.value ?? "violet"
}
...
export function EditAccountModal({
    ...
    const [type, setType] = useState(
        getAccountTypeValue(account.type),
    )
    const [selectedColor, setSelectedColor] = useState(
        getAccountColorValue(account),
    )
